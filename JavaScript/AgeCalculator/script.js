const dobEl = document.getElementById('dob');
    ...
    function calculateYearsOnly(birthDate, currentDate){
      if (birthDate > currentDate) return {error: 'Janam tareekh bhavishya me hai.'};
      let years = currentDate.getFullYear() - birthDate.getFullYear();
      // agar current month/day birth month/day se pehle hai to ek kam karo
      const curMonth = currentDate.getMonth();
      const birthMonth = birthDate.getMonth();
      if (curMonth < birthMonth || (curMonth === birthMonth && currentDate.getDate() < birthDate.getDate())){
        years -= 1;
      }
      return {years};
    }
    ...
    dobEl.addEventListener('keydown', (e) => { if(e.key === 'Enter') btn.click(); });
    curEl.addEventListener('keydown', (e) => { if(e.key === 'Enter') btn.click(); });